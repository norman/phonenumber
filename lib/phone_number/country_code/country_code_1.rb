...
class CountryCode::CountryCode1 < CountryCode::Base
  ...
  class << self
    ...
    def area_code(phone_number)
      phone_number.number.to_s.gsub(/^1/, '').slice(0..2).to_s
    end

    def prefix(phone_number)
      phone_number.number.to_s.gsub(/^1/, '').slice(3..5).to_s
    end

    def line_number(phone_number)
      phone_number.number.to_s.gsub(/^1/, '').slice(6..9).to_s
    end
  ...
  AREA_CODES = {
    # area code => ["state/province", "country", "comment"]
    "201" => ["NJ", "US"],
    "202" => ["DC", "US"],
    "203" => ["CT", "US"],
    "204" => ["MB", "CA"],
